...
def recognize(gear_engine, digit_engine, array):
    ...
    gear_label_scores = gear_engine.ClassifyWithImage(array, threshold=0.5, top_k=3)
    gear_inference_time = time.time() - start_time

    start_time = time.time()
    digit_label_scores = digit_engine.ClassifyWithImage(array, threshold=0.5, top_k=3)
    digit_inference_time = time.time() - start_time

    inference_time = gear_inference_time + digit_inference_time

    # restructure the scores here to keep the interface unchanged for now
    if len(gear_label_scores) == 0 or len(digit_label_scores) == 0:
        return [], inference_time

    gear_label, gear_score = gear_label_scores[0]
    digit_label, digit_score = digit_label_scores[0]

    label_scores = [(10 * gear_label + digit_label, (gear_score + digit_score) / 2)]
...
def worker(gear_model_file, digit_model_file, video_device_index, server_url, d):
    gear_engine = ClassificationEngine(gear_model_file)
    ...
    with video_capture(video_device_index) as camera:
        while True:
            ...
            try:
                ...
                label_scores, inference_time = recognize(gear_engine, digit_engine, image)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gear-model-file', default='models/model_gear_ha.tflite.3_7_2019')
    parser.add_argument('--digit-model-file', default='models/model_digit_ll.tflite.3_7_2019')
    ...
    thread = threading.Thread(target=worker, args=(args.gear_model_file, args.digit_model_file, args.video_device_index, args.server_url, d))
